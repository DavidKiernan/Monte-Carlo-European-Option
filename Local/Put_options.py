...
while user_continue == 'Y':
    try:
        ...
        file_all_option_data = (ticker.upper() + "_Put_Option_Data.csv")  # Auto write to file with ticket value
        df12 = df1.get_put_data(expiry=expiryDate)  # gets all option data on the expriry date or the next closes date
        ...
        temp_file.to_csv("Temp_Put_File.csv")

        put_information = pd.read_csv('Temp_Put_File.csv', sep=',') # sep value between infomation
        find_rows = open("Temp_Put_File.csv", "r")  # reads the file
        reader = csv.reader(find_rows, delimiter=",")  # what separates the columns
        row_count = sum(1 for row in reader)

        result_file = (ticker.upper() + "_MC_Put_Result.csv")  # file that will be written to
        ...
        def put_payoff(asset_price, current_price):
            return max(0.0, current_price - asset_price)

        ind = 0
        put_times = []   # list time append day / 365 as
        while ind < row_count - 1:  # -1 to remove header columns
            for row in put_information:
                strike = put_information.Strike[ind]  # gets the strike price at this row
                strike_price = float(strike)  # uses this stores in the variable
                vol = put_information.IV[ind]
            ...
            for i in range(1, expiry, + 1):   # Inclusive all days
                ...
                for x in range(simulations):
                    ...
                    payoffs.append(put_payoff(asset_price, float(strike_price)))
                    discount_factor = math.exp(-risk_free_rate * time)
                    price = discount_factor * (sum(payoffs) / float(simulations))

            # Write to file for Original
